...
通常、囲碁エンジンと通信するためのプロトコルであるGTPを拡張し、一方が「ホスト」、もう一方が「ゲスト」として接続することで、あたかも対面で打っているかのような操作感を提供します。

人間対人間、AIエンジン対AIエンジン、人間対AIエンジンで対局が可能です。対局開始時には、ローカルとリモートの両方の環境で操作が必要です。またパスと投了を正しく伝達するためにコマンド入力が必要となります。
...
これで、次の相手の着手がパスした側に伝達され盤面に表示され、対局が継続します。パスした側が `F10` を押す前に相手が着手をすると、パスした側に伝達されず対局が中断してしまうことに注意が必要です。通常の着手のあとのパスはこのように処理します。

連続した2回目のパスは、終局となりますので、上記の 1. と 2. のみで 3. は必要ありません。
